...
from model_protein import ESM1b, TransformerConfig

import sys
sys.path.append("/home/beizl42/projects/msa-fewshot/evo")
import esm
from evo.tokenization import Vocab

from utils import visualize_layer_attention, create_sorted_attention_heatmap
# set random seed
...
# model type
model_type = 'gpt2' # 'gpt2' or 'diffusion' or 'esm1b'
...
analyze_attention = True if model_type == 'diffusion' else False
analyze_attention_esm = False
# model
n_layer = 12
n_head = 12
n_embd = 768
...
data_dir = "/home/beizl42/orcd/pool/datasets/uniprotkb"
...
def get_batch(split):
    ...
    if model_type == 'gpt2' or model_type == 'esm1b-ar':
        return x, y
    elif model_type == 'diffusion' or model_type == 'esm1b':
        return x, x
    return x, y
...
elif 'esm1b' in model_type:
    alphabet = esm.data.Alphabet.from_architecture("ESM-1b")
    vocab = Vocab.from_esm_alphabet(alphabet)
    esmconf = TransformerConfig(causal=causal)
    model = ESM1b(vocab=vocab, model_config=esmconf)
else:
    gptconf = GPTConfig(
        sequence_len=model_args['sequence_len'],
        vocab_size=model_args['vocab_size'],
...
while True:
    ...
    if iter_num != 0 and should_sample(iter_num) and master_process and 'esm1b' not in model_type:
        model.eval()
        with torch.no_grad():
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        with ctx:
            if model_type == 'gpt2':
                ...
                # masked language modeling loss
                need_head_weights = True if analyze_attention_esm else False
                results = model(X, need_head_weights=need_head_weights)
                logits = results["logits"]
                # TO-DO
                if analyze_attention_esm:
                    block_attn_wts = results['attentions']  # List of attention weights from each layer
                    wandb.log({"attention/all_layers": wandb.Image(visualize_layer_attention(block_attn_wts))})
            elif model_type == 'esm1b-ar':
                need_head_weights = True if analyze_attention_esm else False
                results = model(X, need_head_weights=need_head_weights)
                logits = results["logits"]
